...
#include <vestio/bnet_plugin/bnet_plugin.hpp>
...
using vestio::public_key_type;
using vestio::chain_id_type;
using vestio::block_id_type;
using vestio::block_timestamp_type;
using std::string;
using vestio::sha256;
using vestio::signed_block_ptr;
using vestio::packed_transaction_ptr;
using std::vector;
...
namespace vestio {
       ...
       //    shared_ptr< vector<char> >  block_msg; ///< packed bnet_message for this block
        ...
        void on( const signed_block_ptr& b ) {
           ...
           if (!b) {
              ...
              VEST_THROW(block_validate_exception, "bad block" );
           }
        ...
        void on( const packed_transaction_ptr& p ) {
           ...
           if (!p) {
              ...
              VEST_THROW(transaction_exception, "bad transaction");
           }
  ...
  class listener : public std::enable_shared_from_this<listener> {
     ...
     public:
        ...
        void run() {
           VEST_ASSERT( _acceptor.is_open(), plugin_exception, "unable top open listen socket" );
           do_accept();
        }
   ...
   void bnet_plugin::plugin_shutdown() {
      ...
      my->for_each_session([](auto ses){
         VEST_ASSERT( false, plugin_exception, "session ${ses} still active", ("ses", ses->_session_num) );
      });
