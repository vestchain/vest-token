...
#include <net/if.h>
#include <vestio/chain/genesis_state.hpp>
...
void
vestd_def::set_host( host_def* h, bool is_bios ) {
  host = h->host_name;
...
struct server_name_def {
  ...
  string vestio_home;
  uint16_t instances;
  server_name_def () : ipaddr(), name(), has_bios(false), vestio_home(), instances(1) {}
};
...
struct launcher_def {
  ...
  producer_set_def producer_set;
   ...
   void assign_name (vestd_def &node, bool is_bios);
  ...
  void prep_remote_config_dir (vestd_def &node, host_def *host);
  void launch (vestd_def &node, string &gts);
  void kill (launch_modes mode, string sig_opt);
  pair<host_def, vestd_def> find_node(uint16_t node_num);
  vector<pair<host_def, vestd_def>> get_nodes(const string& node_number_list);
...
launcher_def::set_options (bpo::options_description &cfg) {
  cfg.add_options()
    ...
    ("skip-signature", bpo::bool_switch(&skip_transaction_signatures)->default_value(false), "nodvest does not require transaction signatures.")
    ("nodvest", bpo::value<string>(&vestd_extra_args), "forward nodvest command line argument(s) to each instance of nodvest, enclose arg in quotes")
    ...
    ("per-host",bpo::value<int>(&per_host)->default_value(0),"specifies how many nodvest instances will run on a single host. Use 0 to indicate all on one.")
...
launcher_def::initialize (const variables_map &vmap) {
  ...
  if( !host_map_file.empty() ) {
    try {
      ...
      for (auto &binding : bindings) {
        for (auto &vestd : binding.instances) {
          vestd.host = binding.host_name;
          vestd.p2p_endpoint = binding.public_name + ":" + boost::lexical_cast<string,uint16_t>(vestd.p2p_port);

          aliases.push_back (vestd.name);
        }
      }
  ...
  char* erd_env_var = getenv ("VESTIO_HOME");
...
void
launcher_def::assign_name (vestd_def &node, bool is_bios) {
...
launcher_def::define_network () {
  ...
  if (per_host == 0) {
    host_def local_host;
    local_host.vestio_home = erd;
    local_host.genesis = genesis.string();
    for (size_t i = 0; i < (total_nodes); i++) {
      vestd_def vestd;
      assign_name(vestd, i == 0);
      aliases.push_back(vestd.name);
      vestd.set_host (&local_host, i == 0);
      local_host.instances.emplace_back(move(vestd));
    }
    ...
    for (size_t i = total_nodes; i > 0; i--) {
      ...
      if (ph_count == 0) {
        ...
        lhost->vestio_home =
          (local_id.contains (lhost->host_name) || servers.default_vestio_home.empty()) ?
          erd : servers.default_vestio_home;
        host_ndx++;
      } // ph_count == 0

      vestd_def vestd;
      assign_name(vestd, do_bios);
      vestd.has_db = false;
      ...
      aliases.push_back(vestd.name);
      vestd.set_host (lhost, do_bios);
      do_bios = false;
      lhost->instances.emplace_back(move(vestd));
      --ph_count;
    } // for i
    bindings.emplace_back( move(*lhost) );
    delete lhost;
...
launcher_def::deploy_config_files (tn_node_def &node) {
  boost::system::error_code ec;
  vestd_def &instance = *node.instance;
  ...
  if (host->is_local()) {
    bfs::path cfgdir = bfs::path(host->vestio_home) / instance.config_dir_name;
    bfs::path dd = bfs::path(host->vestio_home) / instance.data_dir_name;
    ...
    bfs::path rfile = bfs::path (host->vestio_home) / instance.config_dir_name / "config.ini";
    ...
    rfile = bfs::path (host->vestio_home) / instance.config_dir_name / "logging.json";
    ...
    rfile = bfs::path (host->vestio_home) / instance.config_dir_name / "genesis.json";
...
launcher_def::write_config_file (tn_node_def &node) {
  ...
  if (instance.has_db || node.producers.size()) {
    ...
    cfg << "plugin = vestio::producer_plugin\n";
  }
  if( instance.has_db ) {
    cfg << "plugin = vestio::mongo_db_plugin\n";
  }
  if ( p2p == p2p_plugin::NET ) {
    cfg << "plugin = vestio::net_plugin\n";
  } else {
    cfg << "plugin = vestio::bnet_plugin\n";
  }
  cfg << "plugin = vestio::chain_api_plugin\n"
      << "plugin = vestio::history_api_plugin\n";
  cfg.close();
}
...
launcher_def::init_genesis () {
  bfs::path genesis_path = bfs::current_path() / "genesis.json";
   ...
   if (!src.good()) {
      ...
      vestio::chain::genesis_state default_genesis;
...
void
launcher_def::prep_remote_config_dir (vestd_def &node, host_def *host) {
  bfs::path abs_config_dir = bfs::path(host->vestio_home) / node.config_dir_name;
  bfs::path abs_data_dir = bfs::path(host->vestio_home) / node.data_dir_name;

  string acd = abs_config_dir.string();
  string add = abs_data_dir.string();
  string cmd = "cd " + host->vestio_home;

  cmd = "cd " + host->vestio_home;
  if (!do_ssh(cmd, host->host_name)) {
    cerr << "Unable to switch to path " << host->vestio_home
...
void
launcher_def::launch (vestd_def &instance, string &gts) {
  ...
  string vestdcmd = "programs/nodvest/nodvest ";
  if (skip_transaction_signatures) {
    vestdcmd += "--skip-transaction-signatures ";
  }
  if (!vestd_extra_args.empty()) {
    if (instance.name == "bios") {
       ...
       regex r("--plugin +vestio::mongo_db_plugin");
       string args = std::regex_replace (vestd_extra_args,r,"");
       regex r2("--mongodb-uri +[^ ]+");
       args = std::regex_replace (args,r2,"");
       vestdcmd += args + " ";
    }
    else {
       vestdcmd += vestd_extra_args + " ";
    }
  }

  if( add_enable_stale_production ) {
    vestdcmd += "--enable-stale-production true ";
    add_enable_stale_production = false;
  }

  vestdcmd += " --config-dir " + instance.config_dir_name + " --data-dir " + instance.data_dir_name;
  vestdcmd += " --genesis-json " + instance.config_dir_name + "/genesis.json";
  if (gts.length()) {
    vestdcmd += " --genesis-timestamp " + gts;
  }

  if (!host->is_local()) {
    string cmdl ("cd ");
    cmdl += host->vestio_home + "; nohup " + vestdcmd + " > "
    ...
    string cmd = "cd " + host->vestio_home + "; kill -15 $(cat " + pidf.string() + ")";
    format_ssh (cmd, host->host_name, info.kill_cmd);
  }
  else {
    cerr << "spawning child, " << vestdcmd << endl;

    bp::child c(vestdcmd, bp::std_out > reout, bp::std_err > reerr );
    bfs::remove(reerr_sl);
    ...
    if(!c.running()) {
      cerr << "child not running after spawn " << vestdcmd << endl;
      for (int i = 0; i > 0; i++) {
        if (c.running () ) break;
      }
    }
...
#if 0
void
launcher_def::kill_instance(vestd_def, string sig_opt) {
}
#endif
...
vector<pair<host_def, vestd_def>>
launcher_def::get_nodes(const string& node_number_list) {
   vector<pair<host_def, vestd_def>> node_list;
...
launcher_def::bounce (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      const vestd_def& node = node_pair.second;
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.vestio_home + "; "
                 + "export VESTIO_HOME=" + host.vestio_home + string("; ")
                 + "export VESTIO_NODE=" + node_num + "; "
                 + "./scripts/vestio-tn_bounce.sh " + vestd_extra_args;
...
launcher_def::down (const string& node_numbers) {
   ...
   for (auto node_pair: node_list) {
      ...
      string node_num = node.name.substr( node.name.length() - 2 );
      string cmd = "cd " + host.vestio_home + "; "
                 + "export VESTIO_HOME=" + host.vestio_home + "; "
                 + "export VESTIO_NODE=" + node_num + "; "
         + "export VESTIO_TN_RESTART_CONFIG_DIR=" + node.config_dir_name + "; "
                 + "./scripts/vestio-tn_down.sh";
...
int main (int argc, char *argv[]) {
  ...
  cli.add_options()
    ...
    ("down", bpo::value<string>(&down_nodes),"comma-separated list of node numbers that will be taken down using the vestio-tn_down.sh script")
    ("bounce", bpo::value<string>(&bounce_nodes),"comma-separated list of node numbers that will be restarted using the vestio-tn_bounce.sh script")
    ("roll", bpo::value<string>(&roll_nodes),"comma-separated list of host names where the nodes should be rolled to a new version using the vestio-tn_roll.sh script")
  ...
  try {
    ...
    if (vmap.count("version") > 0) {
      cout << vestio::launcher::config::version_str << endl;
      return 0;
    }
...
FC_REFLECT( server_name_def, (ipaddr) (name) (has_bios) (vestio_home) (instances) )

FC_REFLECT( server_identities, (producer) (nonprod) (db) (default_vestio_home) (ssh) )
