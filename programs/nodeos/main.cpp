...
{
   try {
      app().set_version(vestio::nodvest::config::version);
      app().register_plugin<history_plugin>();

      auto root = fc::app_path();
      app().set_default_data_dir(root / "vestio/nodvest/data" );
      app().set_default_config_dir(root / "vestio/nodvest/config" );
      if(!app().initialize<chain_plugin, http_plugin, net_plugin, producer_plugin>(argc, argv))
         return INITIALIZE_FAIL;
      initialize_logging();
      ilog("nodvest version ${ver}", ("ver", app().version_string()));
      ilog("vestio root is ${root}", ("root", root.string()));
      app().startup();
      app().exec();
