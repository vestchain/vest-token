...
BOOST_AUTO_TEST_CASE( forking ) try {
   ...
   auto r2 = c.create_accounts( {N(vestio.token)} );
   wdump((fc::json::to_pretty_string(r2)));
   c.set_code( N(vestio.token), vestio_token_wast );
   c.set_abi( N(vestio.token), vestio_token_abi );
   c.produce_blocks(10);


   auto cr = c.push_action( N(vestio.token), N(create), N(vestio.token), mutable_variant_object()
              ("issuer",       "vestio" )
   ...
   cr = c.push_action( N(vestio.token), N(issue), config::system_account_name, mutable_variant_object()
              ("to",       "dan" )
