...
public:
   ...
   fc::variant get_global_state() {
    ...
    asset get_balance( const account_name& act ) {
         return get_currency_balance(N(vestio.token), symbol(CORE_SYMBOL), act);
    }
...
BOOST_FIXTURE_TEST_CASE( bootseq_test, bootseq_tester ) {
    try {
        ...
        // Create vestio.msig and vestio.token
        create_accounts({N(vestio.msig), N(vestio.token), N(vestio.ram), N(vestio.ramfee), N(vestio.stake), N(vestio.vpay), N(vestio.bpay), N(vestio.saving) });

        // Set code for the following accounts:
        //  - vestio (code: vestio.bios) (already set by tester constructor)
        //  - vestio.msig (code: vestio.msig)
        //  - vestio.token (code: vestio.token)
        set_code_abi(N(vestio.msig), vestio_msig_wast, vestio_msig_abi);//, &vestio_active_pk);
        set_code_abi(N(vestio.token), vestio_token_wast, vestio_token_abi); //, &vestio_active_pk);

        // Set privileged for vestio.msig and vestio.token
        set_privileged(N(vestio.msig));
        set_privileged(N(vestio.token));

        // Verify vestio.msig and vestio.token is privileged
        const auto& vestio_msig_acc = get<account_object, by_name>(N(vestio.msig));
        BOOST_TEST(vestio_msig_acc.privileged == true);
        const auto& vestio_token_acc = get<account_object, by_name>(N(vestio.token));
        BOOST_TEST(vestio_token_acc.privileged == true);


        // Create SYS tokens in vestio.token, set its manager as vestio
        ...
        create_currency(N(vestio.token), config::system_account_name, max_supply);
        // Issue the genesis supply of 1 billion SYS tokens to vestio.system
        issue(N(vestio.token), config::system_account_name, config::system_account_name, initial_supply);
        ...
        // Set vestio.system to vestio
        set_code_abi(config::system_account_name, vestio_system_wast, vestio_system_abi);
        ...
        for( const auto& a : test_genesis ) {
           ...
           r = delegate_bandwidth(N(vestio.stake), a.aname, asset(net), asset(cpu));
           BOOST_REQUIRE( !r->except_ptr );
        }
        ...
        BOOST_TEST(active_schedule.producers.front().producer_name == "vestio");
        ...
        BOOST_REQUIRE_THROW(claim_rewards(N(runnerup1)), vestio_assert_message_exception);
        ...
        BOOST_REQUIRE_THROW(undelegate_bandwidth(N(b1), N(b1), core_from_string("49999500.0000"), core_from_string("49999500.0000")), vestio_assert_message_exception);
