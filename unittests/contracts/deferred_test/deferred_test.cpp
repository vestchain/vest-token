...
#include <vestiolib/transaction.hpp>
#include <vestiolib/dispatcher.hpp>

using namespace vestio;

class deferred_test : public vestio::contract {
   public:
      ...
      void deferfunc( uint64_t payload ) {
         ...
         vestio_assert( payload != 13, "value 13 not allowed in payload" );
      }
...
extern "C" {
    ...
    void apply( uint64_t receiver, uint64_t code, uint64_t action ) {
      if( code == N(vestio) && action == N(onerror) ) {
...
//VESTIO_ABI( deferred_test, (defercall)(deferfunc) )
