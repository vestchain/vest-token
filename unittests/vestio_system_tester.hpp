...
namespace vestio_system {

class vestio_system_tester : public TESTER {
public:

   vestio_system_tester()
   : vestio_system_tester([](TESTER& ) {}){}

   template<typename Lambda>
   vestio_system_tester(Lambda setup) {
      setup(*this);

      produce_blocks( 2 );

      create_accounts({ N(vestio.token), N(vestio.ram), N(vestio.ramfee), N(vestio.stake),
               N(vestio.bpay), N(vestio.vpay), N(vestio.saving), N(vestio.names) });
      ...
      {
         const auto& accnt = control->db().get<account_object,by_name>( N(vestio.token) );
         abi_def abi;
      ...
      BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance("vestio")  + get_balance("vestio.ramfee") + get_balance("vestio.stake") + get_balance("vestio.ram") );
   }
   ...
   action_result buyram( const account_name& payer, account_name receiver, const asset& vestin ) {
      return push_action( payer, N(buyram), mvo()( "payer",payer)("receiver",receiver)("quant",vestin) );
   }
   ...
   asset get_balance( const account_name& act ) {
      vector<char> data = get_row_by_account( N(vestio.token), act, N(accounts), symbol(CORE_SYMBOL).to_symbol_code().value );
   ...
   fc::variant get_stats( const string& symbolname ) {
      auto symb = vestio::chain::symbol::from_string(symbolname);
      auto symbol_code = symb.to_symbol_code().value;
      vector<char> data = get_row_by_account( N(vestio.token), symbol_code, N(stat), symbol_code );
   ...
   fc::variant get_global_state() {
      ...
      return data.empty() ? fc::variant() : abi_ser.binary_to_variant( "vestio_global_state", data, abi_serializer_max_time );
   ...
   abi_serializer initialize_multisig() {
      ...
      {
         create_account_with_resources( N(vestio.msig), config::system_account_name );
         BOOST_REQUIRE_EQUAL( success(), buyram( "vestio", "vestio.msig", core_from_string("5000.0000") ) );
         produce_block();
   ...
   vector<name> active_and_vote_producers() {
      ...
      transfer( "vestio", "alice1111111", core_from_string("650000000.0000"), "vestio" );
      ...
      auto trace_auth = TESTER::push_action(config::system_account_name, updateauth::get_name(), config::system_account_name, mvo()
                                            ...
                                            ("auth",  authority(1, {key_weight{get_public_key( config::system_account_name, "active" ), 1}}, {
                                                  permission_level_weight{{config::system_account_name, config::vestio_code_name}, 1},
...
inline uint64_t M( const string& vest_str ) {
   return core_from_string( vest_str ).get_amount();
}
