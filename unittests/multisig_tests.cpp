...
#include <vestio.msig/vestio.msig.wast.hpp>
#include <vestio.msig/vestio.msig.abi.hpp>
...
#include <vestio.system/vestio.system.wast.hpp>
#include <vestio.system/vestio.system.abi.hpp>
...
class vestio_msig_tester : public tester {
public:

   vestio_msig_tester() {
      create_accounts( { N(vestio.msig), N(vestio.stake), N(vestio.ram), N(vestio.ramfee), N(alice), N(bob), N(carol) } );
      produce_block();
      ...
      set_code( N(vestio.msig), vestio_msig_wast );
      set_abi( N(vestio.msig), vestio_msig_abi );

      produce_blocks();
      const auto& accnt = control->db().get<account_object,by_name>( N(vestio.msig) );
      abi_def abi;
   ...
   void issue( name to, const asset& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(vestio.token), N(issue), manager, mutable_variant_object()
                                ("to",      to )
                                ("quantity", amount )
                                ("memo", "")
                                );
   ...
   void transfer( name from, name to, const string& amount, name manager = config::system_account_name ) {
      base_tester::push_action( N(vestio.token), N(transfer), manager, mutable_variant_object()
                                ("from",    from)
                                ("to",      to )
   ...
   asset get_balance( const account_name& act ) {
      ...
      const auto* tbl = db.find<table_id_object, by_code_scope_table>(boost::make_tuple(N(vestio.token), act, N(accounts)));
      share_type result = 0;
   ...
   transaction_trace_ptr push_action( const account_name& signer, const action_name& name, const variant_object& data, bool auth = true ) {
      ...
      auto trace = base_tester::push_action( N(vestio.msig), name, accounts, data );
      produce_block();
...
transaction vestio_msig_tester::reqauth( account_name from, const vector<permission_level>& auths, const fc::microseconds& max_serialization_time ) {
   fc::variants v;
...
BOOST_AUTO_TEST_SUITE(vestio_msig_tests)

BOOST_FIXTURE_TEST_CASE( propose_approve_execute, vestio_msig_tester ) try {
   ...
   BOOST_REQUIRE_EXCEPTION( push_action( N(alice), N(exec), mvo()
                            ...
                            vestio_assert_message_is("transaction authorization failed")
   );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_unapprove, vestio_msig_tester ) try {
   auto trx = reqauth("alice", {permission_level{N(alice), config::active_name}}, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( propose_approve_by_two, vestio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( propose_with_wrong_requested_auth, vestio_msig_tester ) try {
   auto trx = reqauth("alice", vector<permission_level>{ { N(alice), config::active_name },  { N(bob), config::active_name } }, abi_serializer_max_time );
...
BOOST_FIXTURE_TEST_CASE( big_transaction, vestio_msig_tester ) try {
...
BOOST_FIXTURE_TEST_CASE( update_system_contract_all_approve, vestio_msig_tester ) try {

   // required to set up the link between (vestio active) and (vestio.prods active)
   //
   //                  vestio active
   //                       |
   //             vestio.prods active (2/3 threshold)
   ...
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("vestio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(vestio.prods), config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });

   set_producers( {N(alice),N(bob),N(carol)} );
   produce_blocks(50);

   create_accounts( { N(vestio.token) } );
   set_code( N(vestio.token), vestio_token_wast );
   set_abi( N(vestio.token), vestio_token_abi );

   create_currency( N(vestio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"),
                        get_balance("vestio") + get_balance("vestio.ramfee") + get_balance("vestio.stake") + get_balance("vestio.ram") );

   set_code( config::system_account_name, vestio_system_wast );
   set_abi( config::system_account_name, vestio_system_abi );
   ...
   // execute by alice to replace the vestio system contract
   transaction_trace_ptr trace;
   control->applied_transaction.connect([&]( const transaction_trace_ptr& t) { if (t->scheduled) { trace = t; } } );
   ...
   BOOST_REQUIRE_EXCEPTION( create_account_with_resources( N(alice1111112), config::system_account_name, core_from_string("1.0000"), false ),
                            vestio_assert_message_exception, vestio_assert_message_is("Unknown Test")

   );
} FC_LOG_AND_RETHROW()

BOOST_FIXTURE_TEST_CASE( update_system_contract_major_approve, vestio_msig_tester ) try {

   // set up the link between (vestio active) and (vestio.prods active)
   set_authority(config::system_account_name, "active", authority(1,
      vector<key_weight>{{get_private_key("vestio", "active").get_public_key(), 1}},
      vector<permission_level_weight>{{{N(vestio.prods), config::active_name}, 1}}), "owner",
      { { config::system_account_name, "active" } }, { get_private_key( config::system_account_name, "active" ) });
   ...
   create_currency( N(vestio.token), config::system_account_name, core_from_string("10000000000.0000") );
   issue(config::system_account_name, core_from_string("1000000000.0000"));
   BOOST_REQUIRE_EQUAL( core_from_string("1000000000.0000"), get_balance( "vestio" ) );
   ...
   BOOST_REQUIRE_EXCEPTION(
      ...
      vestio_assert_message_exception, vestio_assert_message_is("transaction authorization failed")
   );
