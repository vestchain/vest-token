...
#include <vestio/chain/permission_object.hpp>
#include <vestio/chain/authorization_manager.hpp>

#include <vestio/chain/resource_limits.hpp>
#include <vestio/chain/resource_limits_private.hpp>
...
BOOST_AUTO_TEST_CASE(link_auths) { try {
   ...
   // Link authority for vestio reqauth action with alice's spending key
   chain.link_authority("alice", "vestio", "spending",  "reqauth");
   ...
   BOOST_CHECK_THROW( chain.link_authority("alice", "vestio", "spending",  "reqauth"), action_validate_exception);

   // Unlink alice with vestio reqauth
   chain.unlink_authority("alice", "vestio", "reqauth");
   ...
   // Link authority for any vestio action with alice's scud key
   chain.link_authority("alice", "vestio", "scud");
...
BOOST_AUTO_TEST_CASE(link_then_update_auth) { try {
   ...
   chain.link_authority("alice", "vestio", "first",  "reqauth");
...
try {
   ...
   // Creating account with vestio. prefix with privileged account
   chain.create_account("vestio.test1");

   // Creating account with vestio. prefix with non-privileged account, should fail
   BOOST_CHECK_EXCEPTION(chain.create_account("vestio.test2", "joe"), action_validate_exception,
                         fc_exception_message_is("only privileged accounts can have names that start with 'vestio.'"));
...
BOOST_AUTO_TEST_CASE( any_auth ) { try {
   ...
   chain.link_authority( "alice", "vestio", "vestio.any", "reqauth" );
   chain.link_authority( "bob", "vestio", "vestio.any", "reqauth" );

   /// this should succeed because vestio::reqauth is linked to any permission
   chain.push_reqauth("alice", { permission_level{N(alice), "spending"} }, { spending_priv_key });
...
try {
   ...
   using resource_usage_object = vestio::chain::resource_limits::resource_usage_object;
   using by_owner = vestio::chain::resource_limits::by_owner;
...
BOOST_AUTO_TEST_CASE( linkauth_special ) { try {
   ...
   auto validate_disallow = [&] (const char *type) {
      BOOST_REQUIRE_EXCEPTION(
         ...
         fc_exception_message_is(std::string("Cannot link vestio::") + std::string(type) + std::string(" to a minimum permission"))
      );
   };
