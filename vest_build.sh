#!/bin/bash
   ...
   if [ "$ARCH" == "Linux" ]; then
      ...
      if [ ! -e /etc/os-release ]; then
         printf "\\n\\tCurrently supports Amazon, Centos, Fedora, Mint & Ubuntu Linux only.\\n"
      ...
      case "$OS_NAME" in
         "Amazon Linux AMI")
            FILE="${SOURCE_DIR}/scripts/vest_build_amazon.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
         ...
         ;;
         "CentOS Linux")
            FILE="${SOURCE_DIR}/scripts/vest_build_centos.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
            MONGOD_CONF=${HOME}/opt/mongodb/mongod.conf
            export LLVM_DIR=${HOME}/opt/wasm/lib/cmake/llvm
         ...
         ;;
         "elementary OS")
            FILE="${SOURCE_DIR}/scripts/vest_build_ubuntu.sh"
         ...
         ;;
         "Fedora")
            FILE="${SOURCE_DIR}/scripts/vest_build_fedora.sh"
            CXX_COMPILER=g++
            C_COMPILER=gcc
   ...
   if [ "$ARCH" == "Darwin" ]; then
      FILE="${SOURCE_DIR}/scripts/vest_build_darwin.sh"
      CXX_COMPILER=clang++
      C_COMPILER=clang
   ...
   if ! "${CMAKE}" -DCMAKE_BUILD_TYPE="${CMAKE_BUILD_TYPE}" -DCMAKE_CXX_COMPILER="${CXX_COMPILER}" \
      ...
      -DCMAKE_INSTALL_PREFIX="/usr/local/vestio" "${SOURCE_DIR}"
   then
   ...
   if [ "${START_MAKE}" == "false" ]; then
      printf "\\n\\t>>>>>>>>>>>>>>>>>>>> has been successfully configured but not yet built.\\n\\n"
      exit 0
   fi
   ...
   printf "\\n\\tHas been successfully built. %02d:%02d:%02d\\n\\n" $(($TIME_END/3600)) $(($TIME_END%3600/60)) $(($TIME_END%60))
