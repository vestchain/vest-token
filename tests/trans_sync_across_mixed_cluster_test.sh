...
killAll()
{
  programs/vestio-launcher/vestio-launcher -k 15
}

cleanup()
{
    rm -rf etc/vestio/node_*
    rm -rf var/lib/node_*
}
...
getHeadBlockNum()
{
  INFO="$(programs/clvest/clvest get info)"
  verifyErrorCode "clvest get info"
...
# stand up nodvest cluster
launcherOpts="-p $pnodes -n $total_nodes -s $topo -d $delay"
echo Launcher options: --nodvest \"--plugin vestio::wallet_api_plugin\" $launcherOpts
programs/vestio-launcher/vestio-launcher --nodvest "--plugin vestio::wallet_api_plugin" $launcherOpts
sleep 7
...
while [ $port2  -ne $endport ]; do
    ...
    TRANS_INFO="$(programs/clvest/clvest --port $port2 get block 1)"
    verifyErrorCode "clvest get block"
    port2=`expr $port2 + 1`
done

# create 3 keys
KEYS="$(programs/clvest/clvest create key)"
verifyErrorCode "clvest create key"
...
PASSWORD_INITA="$(programs/clvest/clvest wallet create --name inita)"
verifyErrorCode "clvest wallet create"
# strip out password from output
PASSWORD_INITA="$(echo "$PASSWORD_INITA" | awk '/PW/ {print $1}')"
# remove leading/trailing quotes
PASSWORD_INITA=${PASSWORD_INITA#\"}
PASSWORD_INITA=${PASSWORD_INITA%\"}
programs/clvest/clvest wallet import --name inita --private-key $INITA_PRV_KEY
verifyErrorCode "clvest wallet import"
programs/clvest/clvest wallet import --name inita --private-key $PRV_KEY1
verifyErrorCode "clvest wallet import"
programs/clvest/clvest wallet import --name inita --private-key $PRV_KEY2
verifyErrorCode "clvest wallet import"
programs/clvest/clvest wallet import --name inita --private-key $PRV_KEY3
verifyErrorCode "clvest wallet import"
...
ACCOUNT_INFO="$(programs/clvest/clvest create account inita testera $PUB_KEY1 $PUB_KEY3)"
verifyErrorCode "clvest create account"
waitForNextBlock
# verify account created
ACCOUNT_INFO="$(programs/clvest/clvest get account testera)"
verifyErrorCode "clvest get account"
...
while [ $port  -ne $endport ]; do
    ...
    TRANSFER_INFO="$(programs/clvest/clvest transfer inita testera 975321 "test transfer")"
    verifyErrorCode "clvest transfer"
   ...
   TRANS_INFO="$(programs/clvest/clvest --port $port2 get transaction $TRANS_ID)"
   verifyErrorCode "clvest get transaction trans_id of <$TRANS_INFO> from node on port $port2"
	port2=`expr $port2 + 1`
    done
