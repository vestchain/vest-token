...
class Node(object):
    ...
    def __init__(self, host, port, pid=None, cmd=None, enableMongo=False, mongoHost="localhost", mongoPort=27017, mongoDb="VESTtest"):
        self.host=host
        self.port=port
        self.pid=pid
        self.cmd=cmd
    ...
    def validateAccounts(self, accounts):
        ...
        for account in accounts:
            ...
            accountInfo=self.getVestAccount(account.name, exitOnError=True)
            try:
    ...
    def getVestAccount(self, name, exitOnError=False):
        assert(isinstance(name, str))
        if not self.enableMongo:
            cmdDesc="get account"
            cmd="%s -j %s" % (cmdDesc, name)
            msg="( getVestAccount(name=%s) )" % (name);
            return self.processCmd(cmd, cmdDesc, silentErrors=False, exitOnError=exitOnError, exitMsg=msg)
        else:
            return self.getVestAccountFromDb(name, exitOnError=exitOnError)

    def getVestAccountFromDb(self, name, exitOnError=False):
        cmd="%s %s" % (Utils.MongoPath, self.mongoEndpointArgs)
    ...
    def verifyAccount(self, account):
        assert(account)
        if not self.enableMongo:
            ret=self.getVestAccount(account.name)
            if ret is not None:
    ...
    def verifyAccountMdb(self, account):
        assert(account)
        ret=self.getVestAccountFromDb(account.name)
        if ret is not None:
    ...
    def transferFunds(self, source, destination, amountStr, memo="memo", force=False, waitForTransBlock=False, exitOnError=True):
        ...
        cmd="%s %s -v transfer -j %s %s" % (
            Utils.VestClientPath, self.endpointArgs, source.name, destination.name)
        cmdArr=cmd.split()
    ...
    def validateFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        currentBalances=self.getVestBalances([source] + accounts)
    ...
    def getVestBalances(self, accounts):
        ...
        balances={}
        for account in accounts:
            balance = self.getAccountVestBalance(account.name)
    ...
    # Get actions mapped to an account (clvest get actions)
    ...
    def getAccountVestBalanceStr(self, scope):
        """Returns SYS currency0000 account balance from clvest get table command. Returned balance is string following syntax "98.0311 SYS". """
        assert isinstance(scope, str)
        amount=self.getTableAccountBalance("vestio.token", scope)
        if Utils.Debug: Utils.Print("getNodeAccountVestBalance %s %s" % (scope, amount))
        assert isinstance(amount, str)
        return amount

    def getAccountVestBalance(self, scope):
        """Returns SYS currency0000 account balance from clvest get table command. Returned balance is an integer e.g. 980311. """
        balanceStr=self.getAccountVestBalanceStr(scope)
        balance=Node.currencyStrToInt(balanceStr)
        return balance

    def getAccountCodeHash(self, account):
        cmd="%s %s get code %s" % (Utils.VestClientPath, self.endpointArgs, account)
    ...
    def publishContract(self, account, contractDir, wasmFile, abiFile, waitForTransBlock=False, shouldFail=False):
        cmd="%s %s -v set contract -j %s %s" % (Utils.VestClientPath, self.endpointArgs, account, contractDir)
    ...
    def pushMessage(self, account, action, data, opts, silentErrors=False):
        cmd="%s %s push action -j %s %s" % (Utils.VestClientPath, self.endpointArgs, account, action)
        cmdArr=cmd.split()
        if data is not None:
            cmdArr.append(data)
        if opts is not None:
    ...
    def processCmd(self, cmd, cmdDesc, silentErrors=True, exitOnError=False, exitMsg=None, returnType=ReturnType.json):
        ...
        cmd="%s %s %s" % (Utils.VestClientPath, self.endpointArgs, cmd)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        trans=None
        try:
    ...
    def getHeadBlockNum(self):
        """returns head block number(string) as returned by clvest get info."""
        if not self.enableMongo:
