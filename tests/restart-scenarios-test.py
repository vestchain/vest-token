...
# --kill-count <nodvest instances to kill>
...
try:
    ...
    Print("Stand up VEST wallet kvestd")
    walletMgr.killall(allInstances=killAll)
    walletMgr.cleanup()
    if walletMgr.launch() is False:
        errorExit("Failed to stand up kvestd.")

    accountsCount=total_nodes
    walletName="MyWallet"
    Print("Creating wallet %s if one doesn't already exist." % walletName)
    wallet=walletMgr.create(walletName, [cluster.vestioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    vestioAccount=cluster.vestioAccount
    ...
    if not cluster.createAccounts(vestioAccount):
    ...
    if cluster.killSomeVestInstances(killCount, killSignal) is False:
        errorExit("Failed to kill Vest instances")
    Print("nodvest instances killed.")
    ...
    if cluster.relaunchVestInstances() is False:
        errorExit("Failed to relaunch Vest instances")
    Print("nodvest instances relaunched.")
    ...
    TestHelper.shutdown(cluster, walletMgr, testSuccessful, killVestInstances, killVestInstances, keepLogs, killAll, dumpErrorDetails)
