...
###############################################################
# nodvest_run_remote_test
#  Tests remote capability of the nodvest_run_test. Test will setup cluster and pass nodes info to nodvest_run_test. E.g.
#  nodvest_run_remote_test.py -v --clean-run --dump-error-detail
###############################################################
...
pnodes=1
total_nodes=pnodes
actualTest="tests/nodvest_run_test.py"
...
try:
    ...
    Print("Starting up %s test: %s" % ("nodvest", actualTest))
    Print("cmd: %s\n" % (cmd))
    ...
    TestHelper.shutdown(cluster, None, testSuccessful, killVestInstances, False, False, killAll, dumpErrorDetails)
