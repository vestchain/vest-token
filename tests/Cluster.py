...
class Cluster(object):
    ...
    # walletd [True|False] Is kvestd running. If not load the wallet plugin
    def __init__(self, walletd=False, localCluster=True, host="localhost", port=8888, walletHost="localhost", walletPort=9899, enableMongo=False
                 , mongoHost="localhost", mongoPort=27017, mongoDb="VESTtest", defproduceraPrvtKey=None, defproducerbPrvtKey=None, staging=False):
        """Cluster container.
        walletd [True|False] Is wallet kvestd running. If not load the wallet plugin
        localCluster [True|False] Is cluster local to host.
        host: vest server host
        port: vest server port
        walletHost: vest wallet host
        walletPort: wos wallet port
        enableMongo: Include mongoDb support, configures vest mongo plugin
        ...
        self.vestioAccount=self.defProducerAccounts["vestio"]= Account("vestio")
    ...
    def launch(self, pnodes=1, totalNodes=1, prodCount=1, topo="mesh", p2pPlugin="net", delay=1, onlyBios=False, dontKill=False
               , dontBootstrap=False, totalProducers=None, extraNodvestArgs=None):
        """Launch cluster.
        ...
        if not self.localCluster:
            Utils.Print("WARNING: Cluster not local, not launching %s." % (Utils.VestServerName))
            return True
        ...
        if not Cluster.arePortsAvailable(set(range(self.port, self.port+totalNodes+1))):
            Utils.Print("ERROR: Another process is listening on nodvest default port.")
            return False

        cmd="%s -p %s -n %s -s %s -d %s -i %s -f --p2p-plugin %s %s" % (
            Utils.VestLauncherPath, pnodes, totalNodes, topo, delay, datetime.datetime.utcnow().strftime("%Y-%m-%dT%H:%M:%S.%f")[:-3],
            p2pPlugin, producerFlag)
        cmdArr=cmd.split()
        if self.staging:
            cmdArr.append("--nogen")

        nodvestArgs="--max-transaction-time 50000 --abi-serializer-max-time-ms 990000 --filter-on * --p2p-max-nodes-per-host %d" % (totalNodes)
        if not self.walletd:
            nodvestArgs += " --plugin vestio::wallet_api_plugin"
        if self.enableMongo:
            nodvestArgs += " --plugin vestio::mongo_db_plugin --mongodb-wipe --delete-all-blocks --mongodb-uri %s" % self.mongoUri
        if extraNodvestArgs is not None:
            assert(isinstance(extraNodvestArgs, str))
            nodvestArgs += extraNodvestArgs
        if Utils.Debug:
            nodvestArgs += " --contracts-console"

        if nodvestArgs:
            cmdArr.append("--nodvest")
            cmdArr.append(nodvestArgs)
        ...
        if nodes is None or totalNodes != len(nodes):
            Utils.Print("ERROR: Unable to validate %s instances, expected: %d, actual: %d" %
                          (Utils.VestServerName, totalNodes, len(nodes)))
            return False
        ...
        self.vestioAccount=self.defProducerAccounts["vestio"]
    ...
    def getClientVersion(verbose=False):
        ...
        try:
            cmd="%s version client" % (Utils.VestClientPath)
    ...
    def createAccountKeys(count):
        ...
        for _ in range(0, count):
            try:
                cmd="%s create key --to-console" % (Utils.VestClientPath)
    ...
    def spreadFunds(self, source, accounts, amount=1):
        ...
        fromm=source
        to=accounts[0]
        Utils.Print("Transfer %s units from account %s to %s on vest server port %d" % (
        ...
        for i in range(0, count):
            ...
            for _ in range(0, count):
                #Utils.Print("nextVestIdx: %d, n: %d" % (nextVestIdx, n))
                nextVestIdx=(nextVestIdx + 1)%count
                if not self.nodes[nextVestIdx].killed:
                    #Utils.Print("nextVestIdx: %d" % (nextVestIdx))
            ...
            #Utils.Print("nextVestIdx: %d, count: %d" % (nextVestIdx, count))
            node=self.nodes[nextVestIdx]
    ...
    def validateSpreadFunds(self, initialBalances, transferAmount, source, accounts):
        ...
        for node in self.nodes:
            ...
            if Utils.Debug: Utils.Print("Validate funds on %s server port %d." %
                                        (Utils.VestServerName, node.port))

            if node.validateFunds(initialBalances, transferAmount, source, accounts) is False:
                Utils.Print("ERROR: Failed to validate funds on vest node port: %d" % (node.port))
                return False
    ...
    def spreadFundsAndValidate(self, transferAmount=1):
        ...
        initialBalances=self.nodes[0].getVestBalances([self.defproduceraAccount] + self.accounts)
        assert(initialBalances)
        assert(isinstance(initialBalances, dict))
    ...
    def validateAccounts(self, accounts, testSysAccounts=True):
        ...
        myAccounts = []
        if testSysAccounts:
            myAccounts += [self.vestioAccount, self.defproduceraAccount, self.defproducerbAccount]
        if accounts:
            assert(isinstance(accounts, list))
            myAccounts += accounts
    ...
    def parseProducers(nodeNum):
        ...
        configFile="etc/vestio/%s/config.ini" % (node)
    ...
    def parseClusterKeys(totalNodes):
        ...
        if producerKeys is None:
            Utils.Print("ERROR: Failed to parse vestio private keys from cluster config files.")
            return None
    ...
    def bootstrap(totalNodes, prodCount, biosHost, biosPort, dontKill=False, onlyBios=False):
        ...
        try:
            ...
            vestioName="vestio"
            vestioKeys=producerKeys[vestioName]
            vestioAccount=Account(vestioName)
            vestioAccount.ownerPrivateKey=vestioKeys["private"]
            vestioAccount.ownerPublicKey=vestioKeys["public"]
            vestioAccount.activePrivateKey=vestioKeys["private"]
            vestioAccount.activePublicKey=vestioKeys["public"]

            if not walletMgr.importKey(vestioAccount, ignWallet):
                Utils.Print("ERROR: Failed to import %s account keys into ignition wallet." % (vestioName))
                return None

            contract="vestio.bios"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(vestioAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
            ...
            for name, keys in producerKeys.items():
                ...
                trans=biosNode.createAccount(initx, vestioAccount, 0)
                if trans is None:
            ...
            if not onlyBios:
                if prodCount == -1:
                    ...
                    with open(setProdsFile, "r") as f:
                        ...
                        opts="--permission vestio@active"
                        myTrans=biosNode.pushMessage("vestio", "setprods", setProdsStr, opts)
                    ...
                    trans=biosNode.pushMessage("vestio", "setprods", setProdsStr, opts)
                ...
                # wait for block production handover (essentially a block produced by anyone but vestio).
                lam = lambda: biosNode.getInfo(exitOnError=True)["head_block_producer"] != "vestio"
                ret=Utils.waitForBool(lam)
                if not ret:
                    Utils.Print("ERROR: Block production handover failed.")
                    return None

            vestioTokenAccount=copy.deepcopy(vestioAccount)
            vestioTokenAccount.name="vestio.token"
            trans=biosNode.createAccount(vestioTokenAccount, vestioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (vestioTokenAccount.name))
                return None

            vestioRamAccount=copy.deepcopy(vestioAccount)
            vestioRamAccount.name="vestio.ram"
            trans=biosNode.createAccount(vestioRamAccount, vestioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (vestioRamAccount.name))
                return None

            vestioRamfeeAccount=copy.deepcopy(vestioAccount)
            vestioRamfeeAccount.name="vestio.ramfee"
            trans=biosNode.createAccount(vestioRamfeeAccount, vestioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (vestioRamfeeAccount.name))
                return None

            vestioStakeAccount=copy.deepcopy(vestioAccount)
            vestioStakeAccount.name="vestio.stake"
            trans=biosNode.createAccount(vestioStakeAccount, vestioAccount, 0)
            if trans is None:
                Utils.Print("ERROR: Failed to create account %s" % (vestioStakeAccount.name))
                return None
            ...
            contract="vestio.token"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            trans=biosNode.publishContract(vestioTokenAccount.name, contractDir, wasmFile, abiFile, waitForTransBlock=True)
            if trans is None:
                Utils.Print("ERROR: Failed to publish contract %s." % (contract))
                return None

            # Create currency0000, followed by issue currency0000
            contract=vestioTokenAccount.name
            Utils.Print("push create action to %s contract" % (contract))
            action="create"
            data="{\"issuer\":\"%s\",\"maximum_supply\":\"1000000000.0000 %s\",\"can_freeze\":\"0\",\"can_recall\":\"0\",\"can_whitelist\":\"0\"}" % (vestioTokenAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push create action to vestio contract.")
                return None
            ...
            action="issue"
            data="{\"to\":\"%s\",\"quantity\":\"1000000000.0000 %s\",\"memo\":\"initial issue\"}" % (vestioAccount.name, CORE_SYMBOL)
            opts="--permission %s@active" % (contract)
            trans=biosNode.pushMessage(contract, action, data, opts)
            if trans is None or not trans[0]:
                Utils.Print("ERROR: Failed to push issue action to vestio contract.")
                return None
            ...
            Utils.Print("Verify vestio issue, Expected: %s" % (expectedAmount))
            actualAmount=biosNode.getAccountVestBalanceStr(vestioAccount.name)
            if expectedAmount != actualAmount:
                Utils.Print("ERROR: Issue verification failed. Excepted %s, actual: %s" %
                            (expectedAmount, actualAmount))
                return None

            contract="vestio.system"
            contractDir="contracts/%s" % (contract)
            wasmFile="%s.wasm" % (contract)
            abiFile="%s.abi" % (contract)
            Utils.Print("Publish %s contract" % (contract))
            ...
            action="transfer"
            for name, keys in producerKeys.items():
                data="{\"from\":\"%s\",\"to\":\"%s\",\"quantity\":\"%s\",\"memo\":\"%s\"}" % (vestioAccount.name, name, initialFunds, "init transfer")
                opts="--permission %s@active" % (vestioAccount.name)
                trans=biosNode.pushMessage(contract, action, data, opts)
                if trans is None or not trans[0]:
                    Utils.Print("ERROR: Failed to transfer funds from %s to %s." % (vestioTokenAccount.name, name))
                    return None
    ...
    # Populates list of VestInstanceInfo objects, matched to actual running instances
    def discoverLocalNodes(self, totalNodes, timeout=0):
        ...
        cmd="pgrep %s %s" % (pgrepOpts, Utils.VestServerName)
        ...
        for i in range(0, totalNodes):
            ...
            if m is None:
                Utils.Print("ERROR: Failed to find %s pid. Pattern %s" % (Utils.VestServerName, pattern))
                break
    ...
    # Kills a percentange of Vest instances starting from the tail and update vestInstanceInfos state
    def killSomeVestInstances(self, killCount, killSignalStr=Utils.SigKillTag):
        killSignal=signal.SIGKILL
        if killSignalStr == Utils.SigTermTag:
            killSignal=signal.SIGTERM
        Utils.Print("Kill %d %s instances with signal %s." % (killCount, Utils.VestServerName, killSignal))
    ...
    def relaunchVestInstances(self):
    ...
    def dumpErrorDetails(self):
        fileName="etc/vestio/node_bios/config.ini"
        Cluster.dumpErrorDetailImpl(fileName)
        fileName="var/lib/node_bios/stderr.txt"
        Cluster.dumpErrorDetailImpl(fileName)

        for i in range(0, len(self.nodes)):
            fileName="etc/vestio/node_%02d/config.ini" % (i)
            Cluster.dumpErrorDetailImpl(fileName)
            fileName="var/lib/node_%02d/stderr.txt" % (i)
            Cluster.dumpErrorDetailImpl(fileName)

    def killall(self, silent=True, allInstances=False):
        """Kill cluster nodvest instances. allInstances will kill all nodvest instances running on the system."""
        cmd="%s -k 9" % (Utils.VestLauncherPath)
        if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
        if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
            if not silent: Utils.Print("Launcher failed to shut down vest cluster.")

        if allInstances:
            # ocassionally the launcher cannot kill the vest server
            cmd="pkill -9 %s" % (Utils.VestServerName)
            if Utils.Debug: Utils.Print("cmd: %s" % (cmd))
            if 0 != subprocess.call(cmd.split(), stdout=Utils.FNull):
                if not silent: Utils.Print("Failed to shut down vest cluster.")
    ...
    def cleanup(self):
        for f in glob.glob("var/lib/node_*"):
            shutil.rmtree(f)
        for f in glob.glob("etc/vestio/node_*"):
            shutil.rmtree(f)
