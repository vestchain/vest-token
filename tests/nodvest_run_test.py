...
try:
    ...
    testWallet=walletMgr.create(testWalletName, [cluster.vestioAccount,cluster.defproduceraAccount,cluster.defproducerbAccount])
    ...
    Print("Create new account %s via %s" % (testeraAccount.name, cluster.vestioAccount.name))
    transId=node.createInitializeAccount(testeraAccount, cluster.vestioAccount, stakedDeposit=0, waitForTransBlock=False, exitOnError=True)

    Print("Create new account %s via %s" % (currencyAccount.name, cluster.vestioAccount.name))
    transId=node.createInitializeAccount(currencyAccount, cluster.vestioAccount, stakedDeposit=5000, exitOnError=True)

    Print("Create new account %s via %s" % (exchangeAccount.name, cluster.vestioAccount.name))
    transId=node.createInitializeAccount(exchangeAccount, cluster.vestioAccount, waitForTransBlock=True, exitOnError=True)
    ...
    actualAmount=node.getAccountVestBalanceStr(testeraAccount.name)
    ...
    Print("Verify transfer, Expected: %s" % (expectedAmount))
    actualAmount=node.getAccountVestBalanceStr(currencyAccount.name)
    if expectedAmount != actualAmount:
        cmdError("FAILURE - transfer failed")
        errorExit("Transfer verification failed. Excepted %s, actual: %s" % (expectedAmount, actualAmount))
    ...
    contractDir="contracts/vestio.token"
    wasmFile="vestio.token.wasm"
    abiFile="vestio.token.abi"
    ...
    if not enableMongo:
        ...
        account=node.getVestAccountFromDb(currencyAccount.name)
        ...
        if abiName != "transfer" or abiActionName != "transfer" or abiType != "transfer":
            errorExit("FAILURE - get VEST account failed", raw=True)
    ...
    account=node.getVestAccount(defproduceraAccount.name, exitOnError=True)
