...
my $vest_home = defined $ENV{VEST_HOME} ? $ENV{VEST_HOME} : getcwd;
my $vestd = $vest_home . "/programs/vestd/vestd";
my $vestc = $vest_home . "/programs/vestc/vestc";

my $nodes = defined $ENV{VEST_TEST_RING} ? $ENV{VEST_TEST_RING} : "1";
my $pnodes = defined $ENV{VEST_TEST_PRODUCERS} ? $ENV{VEST_TEST_PRODUCERS} : "1";

my $prods = 21;
my $genesis = "$vest_home/genesis.json";
...
if (!GetOptions("nodes=i" => \$nodes,
    ...
    print "where:\n";
    print "--nodes=n (default = 1) sets the number of vestd instances to launch\n";
...
sub write_config {
    ...
    if (defined $producer) {
        ...
        print $cfg "private-key = [\"VEST6MRyAjQq8ud7hVNYcfnVPJqcVpscN5So8BhtHuGYqET5GDW5CV\",\"5KQwrPbwdL6PhXujxW37FSSQZ1JiwsST4cqQzDeyXtP79zkvFD3\"]\n";

        print $cfg "plugin = vestio::producer_plugin\n";
        print $cfg "plugin = vestio::chain_api_plugin\n";
...
sub perform_work {
    ...
    if ($mode == 0) {
        ...
        my $counter = 0;
        while (time () < $stoptime) {
            `$vestc transfer vest inita 10 >> vestc.out 2>> vestc.err`;
            $counter++;
            if ($counter % 1000 == 0) {
